...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.setNavigationBarTitle({
      title: s("l") === 0 ? "个人中心" : "Personal Space"
    });
  ...
  onTapDelete: function (e) {
    ...
    let that = this;
    if (!that.data.isStuff && !that.data.formData.studentId == that.data.posts[index].user_id) {
      wx.showToast({
        title: '您无权限执行此操作',
        icon: 'none'
      })
